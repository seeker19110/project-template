...
**Quy trình PR → merge tự động (BẮT BUỘC):** Sau khi **tạo PR**: (1) **đăng ký theo dõi** PR (`subscribe_pr_activity`) và **đặt lịch tự kiểm tra mỗi 3 phút**; **bật auto-merge (squash) ngay sau khi tạo PR** nếu công cụ đang dùng hỗ trợ (vd `enable_pr_auto_merge`/`gh pr merge --auto`) — nền tảng tự merge khi cổng xanh và không xung đột, không cần tự tay canh rồi gọi lệnh merge riêng; công cụ không hỗ trợ thì làm thủ công: (2) khi CI **xanh** → **merge vào `main`** (squash); nếu **đỏ** → sửa rồi kích lại, chưa xanh thì **chưa merge**. (3) **Luôn quay về `main`** sau khi merge (nền làm việc mặc định là `main`). (4) **FIFO — không nhảy cóc:** PR nào **tạo trước merge trước**; nhiều PR mở cùng lúc thì merge lần lượt theo thứ tự tạo, merge xong PR trước (cập nhật lại nhánh sau với `main` nếu cần) mới tới PR kế. (5) **Cập nhật `PROGRESS.md` NGAY sau khi quay về `main`** (SHA đã đối chiếu = HEAD mới, xoá/đổi dòng "Nhánh đang làm" nếu nhánh vừa merge) — đây là nguyên nhân số một khiến `PROGRESS.md` lỗi thời và làm việc lặp lại; job CI `progress-freshness` (`scripts/check-progress-freshness.sh`) chặn merge PR kế tiếp nếu bước này bị bỏ qua. Branch protection thật sự có hiệu lực (không chỉ ghi trong tài liệu) + khớp file khai báo: xem `.github/rulesets/main.json` + job CI `protection-guard` (`docs/ops/repository-settings.md`).
...
- *(Riêng REPO KHUNG này: các mục `[ĐIỀN]` ở §5 và §10 là placeholder CỐ Ý cho dự án đích — repo khung không có `package.json`. Cổng thật của chính repo khung là `scripts/check-docs-consistency.sh`, `scripts/check-ci-policy.sh`, `scripts/check-progress-freshness.sh` và `scripts/test-copy-framework.sh`.)*
