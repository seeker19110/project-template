...
- **Feature gate (BẮT BUỘC).** Với một **tính năng** (không phải sửa lỗi nhỏ/chore): research + `docs/specs/<ngày>-<slug>.md` phải ghi **"Approved for implementation"** kèm người duyệt + ngày **trước khi sửa source code**. Chưa Approved thì chỉ được research/viết spec. Mẫu: `docs/framework/templates/FEATURE-SPEC.template.md`; cổng `pr-policy.yml` kiểm tự động mọi PR `feat`.
