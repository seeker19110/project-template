...
4. Không có `package.json` (vd repo template chưa scaffold) → báo "chưa có hàng rào để chạy", gợi ý `/khoi-tao`, dừng.

## Bước 2 — Chạy & ĐỌC output thật
Chạy từng cổng dò được, **đọc kết quả thật** (không suy đoán). Phạm vi test: trước **commit** chạy test liên quan; trước **merge** chạy **toàn bộ** test (CLAUDE.md §6). Nếu người dùng gõ `/cong merge` → chế độ merge (toàn bộ test + các mục §6).
