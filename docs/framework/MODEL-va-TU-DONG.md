...
| **`/tu-van`** — tư vấn phát triển | Bóc yêu cầu mơ hồ, đề xuất, đánh đổi | 🟡 | ✅ | ✅✅ | ✅✅ |
| **`/khoi-tao`** — khởi tạo dự án mới | Dựng nền, cấu hình hàng rào | 🟡 | ✅ | ✅ | ✅ |
| **`/ui-ux`** — thiết kế UI/UX | Design tokens, mobile-first, WCAG AA, 4 trạng thái | 🟡 | ✅✅ | ✅✅ | ✅✅ |
| **`/cong`** — cổng commit/merge | Chạy build/lint/test, đọc diff, rà rác/bí mật | ✅ | ✅✅ | ✅✅ | ✅✅ |
| **`/adr`** — quyết định kiến trúc | So sánh phương án, hệ quả dài hạn | ❌ | 🟡 | ✅✅ | ✅✅ |
| **`/audit-toi-uu`** — audit & tối ưu | Đo baseline, refactor không đổi hành vi | 🟡 | ✅ | ✅✅ | ✅✅ |
| **`/su-co`** — xử lý sự cố production | Giảm thiệt hại, chẩn đoán, post-mortem | ❌ | 🟡 | ✅✅ | ✅✅ |
| **BO-SUNG-chat-luong** — chống lỗi logic, race/idempotency | Ca biên, tiền tệ, async | 🟡 | 🟡 | ✅✅ | ✅✅ |
| **AP-DUNG (brownfield)** | Đọc repo, suy ra stack thật, nâng tăng dần | 🟡 | ✅ | ✅✅ | ✅✅ |

**Đọc theo nhóm:**
- **Việc code/UI/cổng thường ngày** → **Sonnet 5 đủ tốt → xuất sắc**, chi phí thấp — ngựa thồ.
- **Việc lý luận sâu / rủi ro cao** (KHUNG-3, `/adr`, `/su-co`, chống lỗi logic, audit lớn) → **Opus 4.8 xuất sắc; Sonnet chỉ 🟡** → nâng Opus, hoặc Fable ở ca khó nhất.
...
Các skill nặng suy nghĩ (`/adr`, `/su-co`, `/tu-van`, `/tu-dong` plan mode) đã có **dòng nhắc 💡** ở đầu: gợi ý nâng `/model claude-fable-5` + `/effort xhigh` đúng pha, rồi hạ lại. Đây là **nhắc**, không tự đổi — Claude Code không có cơ chế tự nâng Fable/effort theo độ khó.
...
**Một câu:** Tư vấn + Opus lập kế hoạch toàn bộ → bạn duyệt 1 lần → tự động điều phối tới hoàn thành (Sonnet code, Haiku việc phụ, `thuc-thi` việc rõ phạm vi), với auto-format + cổng chặn commit đỏ, quyền an toàn, tự nhắc dừng khi gần hết quota 5h, phiên sau "tiếp tục". Kích hoạt: gõ **`/tu-dong`**.
...
```
   ...
   (opusplan)    │  (9 giai đoạn / nâng cấp brownfield)│   • tra-cuu (tìm/đọc)
              ...
              EXECUTION  →  SONNET 5 viết code, chạy tự động theo kế hoạch
                                  │  (việc rõ phạm vi → subagent thuc-thi: cô lập + song song)
...
| `thuc-thi.md` | Sonnet | Việc RÕ PHẠM VI đã bóc tách (test theo spec, boilerplate, cập nhật docs, sửa cơ học nhiều file) — cô lập ngữ cảnh + song song, rút tải khỏi main. Không phải "model rẻ hơn" (cùng Sonnet với pha-code). |
...
- **`thuc-thi` cùng Sonnet** với pha-code opusplan — lợi ích là **cô lập ngữ cảnh + song song**, không phải model rẻ hơn.
