...
- Ngày lập: 2026-08-23  ·  Duyệt bởi người dùng: **CHƯA — đang chờ duyệt (Pha 2, cổng §9)**
...
## Definition of Complete (trình duyệt cùng kế hoạch)
- [ ] 0 phát hiện mức **Cao** còn mở; mọi phát hiện Vừa/Thấp có kết cục ghi nhận (sửa / dời / chấp nhận rủi ro).
- [ ] `verify-dropins.sh` PASS **bao gồm cả e2e mẫu** sau khi copy (dropins đầy đủ, dự án đích không mất file nào cấu hình tham chiếu tới).
- [ ] `check-docs-consistency.sh` PASS + CLAUDE.md phản ánh đủ mọi tài liệu luật hiện có (standard-delivery, orchestration, docs/ops mới).
- [ ] CI: mọi action ghim SHA, quyền tối thiểu theo job, PR từ fork không đỏ oan.
- [ ] Chuẩn OSS đủ bộ: CODE_OF_CONDUCT, issue template không trùng lặp, CODEOWNERS cover `.claude/`.
- [ ] `PROGRESS.md` đúng hiện trạng, theo cấu trúc `PROGRESS.template.md`.

## Đợt 1 — Đóng phát hiện CAO: tính toàn vẹn drop-ins (⬜)
...
## Đợt 2 — Bảo mật CI/CD & supply-chain (⬜)
| ID | Từ phát hiện | Việc | Tiêu chí nghiệm thu | Phụ thuộc | Ước lượng | Trạng thái | PR / bằng chứng |
|----|--------------|------|---------------------|-----------|-----------|-----------|-----------------|
...
## Đợt 3 — Chất lượng drop-ins & config (⬜)
| ID | Từ phát hiện | Việc | Tiêu chí nghiệm thu | Phụ thuộc | Ước lượng | Trạng thái | PR / bằng chứng |
|----|--------------|------|---------------------|-----------|-----------|-----------|-----------------|
...
## Đợt 4 — Quản trị OSS & tài liệu đồng bộ (⬜)
| ID | Từ phát hiện | Việc | Tiêu chí nghiệm thu | Phụ thuộc | Ước lượng | Trạng thái | PR / bằng chứng |
|----|--------------|------|---------------------|-----------|-----------|-----------|-----------------|
