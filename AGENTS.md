...
- **Git:** mỗi tính năng một nhánh (`feat/...`, `fix/...`); conventional commits; mọi thay đổi vào `main` qua PR (ưu tiên squash); KHÔNG push thẳng `main`. **Ngay khi tạo PR, cập nhật tài liệu mô tả thay đổi đó (CODEMAP.md, TRAPS.md nếu là bug, ADR nếu đổi kiến trúc…) và commit vào CÙNG PR** — không tách PR riêng theo sau; phát hiện thiếu sau khi đã tạo PR thì push thêm commit vào đúng PR đang mở, không mở PR mới.
